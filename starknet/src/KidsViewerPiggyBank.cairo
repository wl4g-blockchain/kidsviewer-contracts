...
pub trait IKidsViewerPiggyBank<CS> {
    ...
    fn request_withdrawal(
        ref self: CS, person_id: u64, token: ContractAddress, amount: u256, reason: felt252,
    );
    ...
    fn get_person_earnings(self: @CS, person_id: u64, token: ContractAddress) -> (u256, u256);
    ...
    fn is_aave_product_approved(self: @CS, person_id: u64, aave_product: ContractAddress) -> bool;
    ...
    fn aave_product_approvals(self: @CS, person_id: u64, aave_product: ContractAddress) -> bool;
...
pub mod KidsViewerPiggyBank {
    ...
    impl IKidsViewerPiggyBankImpl of super::IKidsViewerPiggyBank<ContractState> {
        ...
        fn request_withdrawal(
            ...
            self
                .emit(
                    WithdrawalRequested { person_id, token, amount, reason, request_id, timestamp },
                );
        }
        ...
        fn reject_withdrawal(ref self: ContractState, request_id: u64, reason: felt252) {
            ...
            let (_, _, _, _, approved, executed, _) = self.withdrawal_requests.read(request_id);
            assert(!approved, 'Already approved');
            assert(!executed, 'Already executed');
            ...
            self
                .emit(
                    WithdrawalRejected {
                        person_id, request_id, reason, timestamp: current_timestamp,
                    },
                );
        }
        ...
        fn recover_all_investments(
            ...
            self
                .emit(
                    InvestmentRecovered { person_id, token, amount: recovered_amount, timestamp },
                );
        }
        ...
        fn set_aave_product_approval(
            ref self: ContractState, person_id: u64, aave_product: ContractAddress, approved: bool,
        ) {
            let caller = get_caller_address();
            let is_approved = self.parent_approvals.read((caller, person_id));
            assert(is_approved, 'Not authorized');
        ...
        fn get_investment_config(self: @ContractState, person_id: u64) -> (bool, u256, u256) {
        ...
        fn person_balances(self: @ContractState, person_id: u64, token: ContractAddress) -> u256 {
            self.person_balances.read((person_id, token))
        }

        fn person_earnings(self: @ContractState, person_id: u64, token: ContractAddress) -> u256 {
        ...
        fn parent_approvals(self: @ContractState, parent: ContractAddress, person_id: u64) -> bool {
            self.parent_approvals.read((parent, person_id))
        }
